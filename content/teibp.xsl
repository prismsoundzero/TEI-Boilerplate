...
<xsl:stylesheet version="1.0"
	...
	<!-- special characters -->
	<xsl:param name="quot"><tei:text>"</tei:text></xsl:param>
	<xsl:param name="apos"><tei:text>'</tei:text></xsl:param>

	<!-- interface text -->
	<xsl:param name="pbNote"><tei:text>page: </tei:text></xsl:param>
	<xsl:param name="altTextPbFacs"><tei:text>view page image(s)</tei:text></xsl:param>
	...
	<xsl:param name="jqueryJS" select="concat($filePrefix,'/js/jquery.min.js')"/>
	<!--<xsl:param name="jqueryBlockUIJS" select="concat($filePrefix,'/js/jquery.blockUI.js')"/>-->
		...
		<head>
			...
			<script type="text/javascript">
				$(document).ready(function() {
					$("html > head > title").text($("TEI > teiHeader > fileDesc > titleStmt > tei-title:first").text());
					//$.unblockUI();
				});
			</script>
	...
	<xsl:variable name="htmlFooter">
		<footer> Powered by <a href="{$teibpHome}">TEI Boilerplate</a>. TEI Boilerplate is licensed under a <a
			...
			Unported License</a>. <a href="http://creativecommons.org/licenses/by/3.0/"><img
				...
				src="https://licensebuttons.net/p/80x15.png"/></a>
		</footer>
	</xsl:variable>
	...
	<xsl:template name="teibpToolbox">
		<div id="teibpToolbox">
			<h1>Options</h1>
			<label for="pbToggle">Masquer les sauts de pages</label>
		...
		<span class="-teibp-pbFacs">
			<a class="gallery-facs" rel="prettyPhoto[gallery1]">
				<xsl:attribute name="onclick">
					<xsl:value-of select="concat('showFacs(',$apos,$n,$apos,',',$apos,$facs,$apos,',',$apos,$id,$apos,',',$apos,$filePrefix,$apos,')')"/>
				</xsl:attribute>
